...
- アプリ起動のたびにランダムな合言葉を作り、QRコード用URLに含めます。
- 合言葉が一致しないアクセスには画面を配信しません。同じWi-Fi内の第三者がMacのIPアドレスとポート番号を探し当てても、合言葉を知らなければ画面は見えません。
- 合言葉はアプリを起動し直すと変わります。古いQRコードは使えなくなるので、表示できない時はQRコードを読み取り直してください。
...
URL末尾の `?key=` から始まる部分（合言葉）まで含めて、全部そのまま入力する必要があります。
