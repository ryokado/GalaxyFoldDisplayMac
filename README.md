...
> このアプリ単体では、macOSに外部ディスプレイを追加しません。
> BetterDisplayなどでMac側に仮想ディスプレイを作り、その画面をGalaxy Foldへ配信することで、拡張ディスプレイ相当の使い方をします。
...
- BetterDisplayなどで作った仮想ディスプレイを直接選んで配信する
- 高速配信中の送信枚数と直近の画像サイズを表示する
- 表示設定を「速さ」「標準」「画質」から選ぶ
...
1. BetterDisplayでMac側に仮想ディスプレイを作る
2. macOSの「システム設定」→「ディスプレイ」で、仮想ディスプレイが追加されていることを確認する
3. Macアプリで「画面一覧を更新」を押す
4. BetterDisplayの仮想画面を選ぶ
5. 「直接配信開始」を押す
6. Galaxy FoldでQRコードを読み取る

これで、Mac上では仮想ディスプレイが拡張画面として扱われ、その画面がGalaxy FoldのChromeに表示されます。
...
直接配信の表示が「高速配信」になっていれば、Macの高速な画面取得を使えています。
「低速配信」になっている場合は、古い取得方法に戻っているためカクつきやすくなります。
...
- BetterDisplay連携の使いやすさを改善する
- 固定の署名を設定して、画面収録の再許可を減らす
- 配布用の署名設定を整える
